...
-- Add commission_multiplier column
ALTER TABLE public.aaa_assignments
ADD COLUMN commission_multiplier numeric NOT NULL DEFAULT 1;

-- Update complete_aaa_task to apply multiplier
...
BEGIN
  ...
  -- Step 1: Process each car, accumulate raw commission
  ...
  -- Step 2: Apply multiplier to total commission
  ...
  IF _all_completed THEN
    -- All cars done: escrow + commission go straight to balance
    _new_balance := ROUND(_new_balance + _total_commission, 2);
  ...
  ELSE
    -- Partial: hold raw commission in escrow (multiplier applied on final release)
    _new_escrow := ROUND(_profile.escrow_balance + _raw_commission, 2);
...
-- Update submit_pending_task to apply multiplier on final release
...
BEGIN
  ...
  IF _all_completed THEN
    -- Sum all raw commissions in escrow, apply multiplier once
    _total_raw_commission := _new_escrow;
    _final_commission := ROUND(_total_raw_commission * _multiplier, 2);
    _new_balance := ROUND(_new_balance + _final_commission, 2);
