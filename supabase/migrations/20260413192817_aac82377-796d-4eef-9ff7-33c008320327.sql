...
BEGIN
  ...
  -- Process each car sequentially: deduct ALL prices, track which were affordable
  FOR _i IN 1..array_length(_aaa.car_prices, 1) LOOP
    ...
    ELSE
      -- User CANNOT afford - still deduct, balance goes negative
      _new_balance := ROUND(_new_balance - _car_price, 2);
  ...
  ELSE
    -- Partial: balance is now negative, escrow holds completed car values
    UPDATE public.profiles
    SET balance = _new_balance,
