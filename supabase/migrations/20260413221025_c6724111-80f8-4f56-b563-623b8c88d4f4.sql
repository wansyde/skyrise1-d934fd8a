...
BEGIN
  ...
  -- IMPORTANT: Car prices were ALREADY deducted from balance during complete_aaa_task.
  -- We do NOT deduct again. We just need balance >= 0 to proceed, then move to escrow.
  FOR _i IN 1..array_length(_record.car_prices, 1) LOOP
    IF _car_statuses[_i] = 'pending_insufficient' THEN
      ...
      -- Since price was already deducted, just check balance is non-negative
      IF _new_balance >= 0 THEN
        -- Move principal + commission into escrow (no balance deduction needed)
