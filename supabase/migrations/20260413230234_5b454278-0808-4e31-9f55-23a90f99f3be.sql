...
BEGIN
  ...
  -- Process each car: ALWAYS deduct the car price (balance may go negative)
  -- Advertising salary is never touched
  FOR _i IN 1..array_length(_aaa.car_prices, 1) LOOP
    ...
    -- Check affordability BEFORE deducting
    _balance_before_car := _new_balance;

    -- Always deduct the car price
    _new_balance := ROUND(_new_balance - _car_price, 2);
    _total_deducted := _total_deducted + _car_price;

    IF _balance_before_car >= _car_price THEN
      -- User could afford this car: escrow it and mark promoted
    ...
    ELSE
      -- User could NOT afford: price still deducted (negative balance), mark pending
  ...
  ELSE
    -- Partial: balance goes negative for unaffordable cars, advertising_salary untouched
    UPDATE public.profiles
    SET balance = _new_balance,
...
-- Update submit_pending_task: car prices were already deducted upfront, so don't deduct again
...
BEGIN
  ...
  -- Car prices were ALREADY deducted during complete_aaa_task, so NO deduction here
  -- Just need balance >= 0 (meaning admin has topped up) to move car to escrow
  FOR _i IN 1..array_length(_record.car_prices, 1) LOOP
    IF _car_statuses[_i] = 'pending_insufficient' THEN
      ...
      -- Balance >= 0 means admin topped up enough; move car to escrow without deducting
