...
BEGIN
  ...
  -- Set targets by tier and set number
  CASE _profile.vip_level
    WHEN 'Elite' THEN
  ...
  -- Deterministic set target based on user+day+set seed
  ...
  -- Simple approach: average reward per remaining task, with random jitter
  IF _remaining_tasks_in_set <= 0 OR _budget_left <= 0 THEN
    _reward := 0;
  ELSIF _remaining_tasks_in_set = 1 THEN
    -- Last task: give exactly what's needed to hit target (capped at max)
  ...
  ELSE
    ...
    -- Jitter: ±30% randomness using a hash-based seed per task
    ...
    -- Never exceed budget or max target
