...
-- Add initial_deposit column
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS initial_deposit numeric NOT NULL DEFAULT 0;

-- Update admin_deposit to record initial_deposit on first deposit
...
BEGIN
  ...
  SET balance = _new_balance,
      initial_deposit = CASE WHEN initial_deposit = 0 THEN _amount ELSE initial_deposit END,
      updated_at = now()
...
-- Update preview_task: Junior uses initial_deposit-based targets
...
BEGIN
  ...
  -- Junior: targets based on initial_deposit (9%-10% per set)
  ...
  CASE _profile.vip_level
    ...
    ELSE
      -- Junior: 9%-10% of starting deposit per set
...
-- Update complete_task: Junior uses initial_deposit-based targets
