...
-- Add decimal noise to effective_amount in both preview_task and complete_task
-- Uses deterministic seed so preview matches complete
...
BEGIN
  ...
  -- Deterministic decimal noise: natural-looking task values
...
BEGIN
  ...
  -- Deterministic decimal noise: natural-looking task values (same seed as preview_task)
  _noise_seed := _user_id::text || ':tv:' || _profile.tasks_completed_today::text || ':' || _current_set::text;
  _noise_byte1 := get_byte(decode(substr(md5(_noise_seed), 5, 2), 'hex'), 0);
  _noise_byte2 := get_byte(decode(substr(md5(_noise_seed), 7, 2), 'hex'), 0);
  _decimal_noise := 0.01 + (_noise_byte1::numeric / 255) * 0.98;
