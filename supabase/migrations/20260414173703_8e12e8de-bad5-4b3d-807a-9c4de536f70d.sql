...
-- 1. Update admin_deposit to always overwrite initial_deposit
...
-- 2. Update preview_task with improved distribution (last 30% high-value) and profit enforcement
...
DECLARE
  ...
  _is_high_zone BOOLEAN;
  _high_zone_start INTEGER;
BEGIN
  _user_id := auth.uid();
  ...
  -- Determine if we're in the high-value zone (last 30% of set)
  _high_zone_start := CEIL(_tasks_per_set * 0.70);
  _is_high_zone := (_tasks_in_set_before >= _high_zone_start);

  IF _remaining_tasks_in_set <= 0 OR _budget_left <= 0 THEN
    _reward := 0;
  ELSIF _remaining_tasks_in_set <= 5 AND _budget_left > (_avg_reward * 0.5) THEN
    -- Profit enforcement: boost last 3-5 tasks if behind target
    ...
    _jitter := (get_byte(decode(substr(md5(_seed_text), 3, 2), 'hex'), 0)::numeric / 255) * 0.2 - 0.1;
  ...
  -- If in high-value zone, push task value to 80-98% of balance
  IF _is_high_zone AND _profile.balance > 0 THEN
    _noise_seed := _user_id::text || ':hz:' || _profile.tasks_completed_today::text;
    _noise_byte1 := get_byte(decode(substr(md5(_noise_seed), 1, 2), 'hex'), 0);
    _effective_amount := GREATEST(
      _effective_amount,
      ROUND(_profile.balance * (0.80 + (_noise_byte1::numeric / 255) * 0.18), 2)
    );
    -- Recalculate reward from the higher task value
    _reward := ROUND(_effective_amount * _tier_percent, 2);
    _reward := LEAST(_reward, _budget_left, _set_max_target - _set_profit_so_far);
    _reward := GREATEST(_reward, 0);
    -- Re-derive effective amount from capped reward
    _effective_amount := CASE WHEN _tier_percent > 0 THEN ROUND(_reward / _tier_percent, 2) ELSE 0 END;
  END IF;
...
-- 3. Update complete_task with same distribution logic
...
BEGIN
  ...
  -- High-value zone: last 30% of tasks
  _high_zone_start := CEIL(_tasks_per_set * 0.70);
  _is_high_zone := (_tasks_in_set_before >= _high_zone_start);

  IF _remaining_tasks_in_set <= 0 OR _budget_left <= 0 THEN
    _reward := 0;
  ELSIF _remaining_tasks_in_set <= 5 AND _budget_left > 0 THEN
    -- Profit enforcement: boost last 3-5 tasks to hit target
    _avg_reward := _budget_left / _remaining_tasks_in_set;
    _seed_text := _user_id::text || ':' || _profile.tasks_completed_today::text || ':' || to_char(COALESCE(_profile.last_task_reset, now()) AT TIME ZONE 'UTC', 'YYYYMMDDHH24MISS');
    _jitter := (get_byte(decode(substr(md5(_seed_text), 3, 2), 'hex'), 0)::numeric / 255) * 0.2 - 0.1;
    _reward := _avg_reward * (1 + _jitter);
  ...
  -- High-value zone: push task value to 80-98% of balance
  IF _is_high_zone AND _profile.balance > 0 THEN
    _noise_seed := _user_id::text || ':hz:' || _profile.tasks_completed_today::text;
    _noise_byte1 := get_byte(decode(substr(md5(_noise_seed), 1, 2), 'hex'), 0);
    _effective_amount := GREATEST(
