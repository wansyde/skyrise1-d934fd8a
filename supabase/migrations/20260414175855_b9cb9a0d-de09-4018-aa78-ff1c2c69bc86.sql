...
BEGIN
  ...
  -- Accumulated profit so far in this set
  ...
  -- Minimum task value: no tiny amounts
  ...
  ELSIF _remaining_tasks_in_set <= 5 THEN
    -- Boost last few tasks to hit target
  ...
  -- Zone-based task value distribution using deterministic seed
  ...
  IF _profile.balance > 0 THEN
    IF _zone_roll < 0.25 THEN
      ...
      _effective_amount := GREATEST(_effective_amount, ROUND(_zone_min + (_noise_byte1::numeric / 255) * (_zone_max - _zone_min), 2));
    ELSIF _zone_roll < 0.70 THEN
      -- MID zone: 0.35B → 0.75B
      _noise_byte1 := get_byte(decode(substr(md5(_noise_seed), 3, 2), 'hex'), 0);
      _zone_min := 0.35 * _profile.balance;
      _zone_max := 0.75 * _profile.balance;
      _effective_amount := GREATEST(_effective_amount, ROUND(_zone_min + (_noise_byte1::numeric / 255) * (_zone_max - _zone_min), 2));
    ELSE
      -- HIGH zone: 0.75B → 0.98B
      _noise_byte1 := get_byte(decode(substr(md5(_noise_seed), 3, 2), 'hex'), 0);
    ...
    -- Recalculate reward from zone-adjusted task value
