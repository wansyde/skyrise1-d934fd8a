...
BEGIN
  ...
  -- ======= ZONE-BASED RANDOM TASK VALUE =======
  _min_task := GREATEST(30, ROUND(_profile.balance * 0.15, 2));

  -- Determine zone: 30% LOW, 40% MID, 30% HIGH
  ...
  IF _zone_rand < 0.30 THEN
    -- LOW zone: 25% to 45% of balance
    _zone_low := 0.25;
    _zone_high := 0.45;
  ELSIF _zone_rand < 0.70 THEN
  ...
  -- Derive display value to match capped reward exactly
...
BEGIN
  ...
  -- Last task boost to hit 10% minimum
