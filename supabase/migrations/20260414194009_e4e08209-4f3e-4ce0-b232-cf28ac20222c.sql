...
BEGIN
  ...
  -- ======= PACING CONTROL =======
  -- progress_ratio: how far through the set (0.0 to 1.0)
  _progress_ratio := _tasks_in_set_before::numeric / _tasks_per_set::numeric;
  -- expected_profit at this point (linear trajectory toward min_target)
  _expected_profit := _progress_ratio * _set_target;
  -- pace_diff: positive = ahead of schedule, negative = behind
  ...
  -- Shift zone thresholds based on pacing
  -- _zone_bias: how much to shift toward LOW (positive) or HIGH (negative)
  -- Clamp pace_diff relative to set_target for proportional response
  ...
  -- Base thresholds: LOW < 0.30, MID < 0.70, HIGH >= 0.70
  -- When ahead (_zone_bias > 0): expand LOW zone, shrink HIGH
  -- When behind (_zone_bias < 0): shrink LOW zone, expand HIGH
  IF _zone_rand < (0.30 + _zone_bias) THEN
    -- LOW zone: 25% to 45% of balance
    _zone_low := 0.25; _zone_high := 0.45;
  ELSIF _zone_rand < (0.70 + _zone_bias * 0.5) THEN
    -- MID zone: 45% to 75% of balance
    _zone_low := 0.45; _zone_high := 0.75;
  ELSE
    -- HIGH zone: 75% to 98% of balance
    _zone_low := 0.75; _zone_high := 0.98;
  END IF;

  -- Pick value within zone
  ...
  -- Hard cap at 12%
  _max_reward := ROUND(_base_deposit * 0.12, 2) - _salary_so_far;
...
BEGIN
  ...
  -- Last task boost to guarantee 10% minimum
