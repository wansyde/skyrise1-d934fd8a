...
-- Update complete_task with safety caps
...
BEGIN
  ...
  -- SAFETY: cap base deposit to prevent inflated profit targets when initial_deposit is stale
  ...
  -- Store normal reward for safety cap reference
  _normal_reward := _reward;
  ...
  -- SAFETY: No single task reward can exceed 5x the normal per-task reward
  -- This prevents catastrophic spikes from stale initial_deposit or timestamp issues
  IF _reward > _normal_reward * 5 AND _normal_reward > 0 THEN
    _reward := ROUND(_normal_reward * 5, 2);
  END IF;
  _reward := GREATEST(_reward, 0.01);
...
-- Update preview_task with same safety caps
