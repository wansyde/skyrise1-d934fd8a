...
BEGIN
  ...
  IF _amount < 50 THEN
    RETURN json_build_object('error', 'Minimum withdrawal amount is $50');
  END IF;
...
-- Also update the legacy single-arg overload for safety
...
BEGIN
  ...
  IF _amount > _profile.balance THEN RETURN json_build_object('error', 'Insufficient balance'); END IF;
  IF _wallet_address IS NULL OR TRIM(_wallet_address) = '' THEN
    RETURN json_build_object('error', 'Wallet address is required');
  END IF;
